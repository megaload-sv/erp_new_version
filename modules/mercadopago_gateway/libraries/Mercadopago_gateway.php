...
class Mercadopago_gateway extends App_gateway
{
    ...
    public function process_payment($data)
    {
        ...
        $test_mode_enabled = $this->ci->db->select('tbloptions.*');
        $this->ci->db->from('tbloptions');
        $this->ci->db->where('name','paymentmethod_mercadopago_test_mode_enabled');
        $test_mode_enabled = $this->ci->db->get()->result()[0]->value;
        ...
        $user =  $this->ci->db->select('tblcontacts.*');
        $this->ci->db->from('tblcontacts');
        ...
        $preference_data = array(
              ...
              "back_urls" => array(
                ...
                "success" => site_url('mercadopago_gateway/process/complete_purchase'),
