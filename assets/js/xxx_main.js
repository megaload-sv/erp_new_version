...
if ($('.f_client_id').length > 0) {
    var a =$('label').filter(function () {//--EDX--
        return $.trim(this.firstChild.nodeValue).toLowerCase() === 'forma de pago';
    }).attr('for');
    var b =$('label').filter(function () {//--EDX--
        return $.trim(this.firstChild.nodeValue).toLowerCase() === 'forma de pago descripción';
    }).attr('for');
    var cts =$('label').filter(function () {//--EDX--
        return $.trim(this.firstChild.nodeValue).toLowerCase() === 'contactos';
    }).attr('for');
    contacts = $('select[name ="'+cts+'[]"]').closest('div'); $('select[name ="'+cts+'[]"]').closest('div').remove();
    contacts.insertAfter('.f_client_id');
    if ($('select[name ="'+a+'"]').val() == 'Otro'){
        $('input[name ="'+b+'"]').closest('div').show();
    }else{
        $('input[name ="'+b+'"]').val('');
        $('input[name ="'+b+'"]').closest('div').hide();
    }
}
...
$(function() {
    ...
    $('td button').click(function(event) {
            $(this).closest('td').find(".dots").toggle();
            $(this).closest('td').find("textarea.dots").focus();
            $(this).find("textarea.dots").removeClass("dots");
        })

    $('td textarea.dhdn').focusout(function(event) {
        $(this).addClass("dots");
        $(this).closest('td').find(".dots").toggle();
    })
    ...
    $("body").on('change', '.f_client_id select[name="clientid"]', function() {
        ...
        requestGetJSON('invoices/client_change_data/' + val + '/' + currentInvoiceID).done(function(response) {
            ...
            //edx
            ccontext="invoice";
            if ($('select[data-fieldto="estimate"]').length>0) {ccontext = "estimate"}
            if ($('select[data-fieldto="invoice"]').length>0) {ccontext = "invoice"}
            $('select[data-fieldto="estimate"]').length;
            cfield = $('select.selectpicker.form-control.custom-field-multi-select[data-fieldto="estimate"]').data('fieldid');
            init_client_contact($('select[name="custom_fields['+ccontext+']['+cfield+'][]"]'), response.client_contacts_edx);
        });
...
function init_client_contact(edx00, cContacs) { //--EDX--
    var edx01 = edx00.find('option');
    if(edx01.length > 0){
    edx00.find('option').not(':first').remove();
    $.each(cContacs, function( i, elm ) {
        idx= elm.split(' / ');
        edx00.append($('<option>', {value:idx[2],text: idx[0]}));
    });
    edx00.selectpicker('refresh');
    }
}