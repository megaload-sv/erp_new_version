...
$fecha = utf8_encode( strftime("%A, %d de %B de %Y",  strtotime($estimate->datecreated)));
//setlocale(LC_ALL,NULL);

$top_header = '<table cellpadding="6" width="100%">
<tr>
    <td align="middle" width="30%" >'.pdf_logo_url().'</td>
    <td align="center" width="70%" style="font-size:15px;" >'.format_organization_info().'</td>
</tr>
</table>';
      $preferer_contact="";
...
$subject_0 = '<span>Señores:</span><br>                
      <span><b>'.strtoupper( $estimate->client->company ).'</b></span><br>
      <span>Atencion: <b>'.substr($preferer_contact, 0,-4).'
       </b></span><br><br>
<span>Estimados Señores:</span><br><br>
<span>Reciban un cordial saludo y nuestros mejores deseos en sus proyectos empresariales.<br> <br>
Por ese medio ponemos a su consideración los servicios solicitados según sus requerimientos: </span><br><br>
<span>'.  ucfirst( $estimate->adminnote ).'</span>';

$info_right_column .= '<span style="font-weight:bold;font-size:17px; text-align:center;">' . _l('estimate_pdf_heading') . '</span><br />';
$info_right_column .= '<span style="font-weight:bold;font-size:22px; text-align:center;"> ' . $estimate_number . '</span>';

if (get_option('show_status_on_pdf_ei') == 1) {
    $info_right_column .= '<br /><span style="color:rgb(' . estimate_status_color_pdf($status) . ');text-transform:uppercase; text-align:center;">' . format_estimate_status($status, '', false) . '</span>';
}

// Add logo
$info_left_column .= pdf_logo_url();
// Write top left logo and right column info/text
pdf_multi_row($top_header, $info_right_column, $pdf, 3*($dimensions['wk'] / 4) );
$pdf->ln(2);
pdf_multi_row("&nbsp;", $fecha, $pdf, 3*($dimensions['wk'] / 6) );
//$pdf->ln(2);
...
$pdf->writeHTML($subject_0 , true, false, false, false, '');
...
$pdf->Ln(-3);
$tbltotal = '';
$tbltotal .= '<table cellpadding="6" width="98%" style="font-size:' . ($font_size + 4) . 'px">';
/*$tbltotal .= '
<tr>
...
if (!empty($estimate->terms)) {
    $pdf->Ln(1);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('terms_and_conditions'), 0, 1, 'C', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(1);
