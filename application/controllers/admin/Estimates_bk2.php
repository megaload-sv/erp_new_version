...
class Estimates extends AdminController
{
    ...
    public function get_estimate_data_ajax($id, $to_return = false)
    {
        ...
        $estimate->preferer_contact = get_preferer($estimate->id,  "estimate_contactos", "estimate", $estimate->clientid ); //--EDX-- Nueva Helper

        $estimate->contacts = $this->clients_model->get_contacts($estimate->clientid);
    ...
    public function convert_to_invoice($id)
    {
        ...
        $invoiceid = $this->estimates_model->convert_to_invoice($id, false, $draft_invoice, $invoice_type);
        if ($invoiceid) {
            set_alert('success', _l('estimate_convert_to_invoice_successfully'));
            if($invoice_type == "credito"){
    ...
    public function pdf($id)
    {
        ...
        $estimate        = $this->estimates_model->get($id);
        $estimate->preferer_contact = get_preferer($estimate->id,  "estimate_contactos", "estimate", $estimate->clientid ); 
        ...
        $fileNameHookData = hooks()->apply_filters('estimate_file_name_admin_area', [
                    'file_name' => $estimate_number." ".$estimate->client->company." ".strtoupper(date("d-M",strtotime( $estimate->date)))." ".$estimate->adminnote . '.pdf',
                    'estimate'  => $estimate,
                    ]);
