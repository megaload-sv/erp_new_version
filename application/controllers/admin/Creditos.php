...
class Creditos  extends AdminController
{
    ...
    {
        $this->list_creditos($id);
    }

    /* List all invoices datatables */
    public function list_creditos($id = '')
    {
        ...
        $data['invoiceid']            = $id;
        $data['title']                = _l('credito');
        $data['invoices_years']       = $this->creditos_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->creditos_model->get_sale_agents();
        $data['invoices_statuses']    = $this->creditos_model->get_statuses();
        $data['bodyclass']            = 'invoices-total-manual';
        $data['creditos_view']        = true;
        $this->load->view('admin/creditos/manage', $data);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Invoices_model::STATUS_CANCELLED) ? $this->creditos_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/invoices/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != Invoices_model::STATUS_CANCELLED) ? $this->creditos_model->get_expenses_to_bill($customer_id) : [];
    ...
    public function validate_credito_number()
    {
    ...
    {
        if ($this->input->post() && user_can_view_credito($rel_id)) {
    ...
    public function pause_overdue_reminders($id)
    {
        ...
        admin_url('creditos/list_creditos/' . $id);
    }
    ...
    public function mark_as_cancelled($id)
    {
        ...
        $success = $this->creditos_model->mark_as_cancelled($id);
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->creditos_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    public function copy($id)
    {
        ...
        $new_id = $this->creditos_model->copy($id);
        if ($new_id) {
    ...
    {
        $invoice = $this->creditos_model->get($id);
        $cf      = get_custom_fields('items');

        $i = 0;

        foreach ($invoice->items as $item) {
            $invoice->items[$i]['taxname']          = get_credito_item_taxes($item['id']);
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->creditos_model->add($invoice_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('invoice')));
                    $redUrl = admin_url('creditos/list_creditos/' . $id);
                ...
                $success = $this->creditos_model->update($invoice_data, $id);
                if ($success) {
        ...
        if ($id == '') {
            ...
            $data['invoices_to_merge'] = $this->creditos_model->check_for_merge_invoice($invoice->clientid, $invoice->id);
            $data['expenses_to_bill']  = $this->creditos_model->get_expenses_to_bill($invoice->clientid);

            $data['invoice']        = $invoice;
            $data['edit']           = true;
            $data['billable_tasks'] = $this->tasks_model->get_billable_tasks($invoice->clientid, !empty($invoice->project_id) ? $invoice->project_id : '');

            $title = _l('edit', _l('invoice_lowercase')) . ' - ' . format_credito_number($invoice->id);
        }
    ...
    public function get_credito_data_ajax($id)
    {
        if (!has_permission('invoices', '', 'view')
            && !has_permission('invoices', '', 'view_own')
            && get_option('allow_staff_view_invoices_assigned') == '0') {
        ...
        $data['invoices_to_merge']          = $this->creditos_model->check_for_merge_invoice($invoice->clientid, $id);
        $data['members']                    = $this->staff_model->get('', ['active' => 1]);
        $data['payments']                   = $this->payments_model->get_invoice_payments($id);
        $data['activity']                   = $this->creditos_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows(db_prefix() . 'notes', ['rel_id' => $id, 'rel_type' => 'invoice']);
        $data['invoice_recurring_invoices'] = $this->creditos_model->get_invoice_recurring_invoices($id);
    ...
    public function apply_credits($invoice_id)
    {
        ...
        if ($total_credits_applied > 0) {
            update_credito_status($invoice_id, true);
            set_alert('success', _l('invoice_credits_applied'));
        }
        redirect(admin_url('creditos/list_creditos' . $invoice_id));
    }

    public function get_creditos_total()
    {
        if ($this->input->post()) {
            load_creditos_total_template();
        }
    }

    /* Record new inoice payment view */
    public function record_credito_payment_ajax($id)
    {
        $this->load->model('payment_modes_model');
        $this->load->model('payments_model');
        $data['payment_modes'] = $this->payment_modes_model->get('', [
        ...
        $data['invoice']  = $this->creditos_model->get($id);
        $data['payments'] = $this->payments_model->get_credito_payments($id);
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('creditos/list_creditos' . $this->input->post('invoiceid')));
        }
    }
    ...
    public function send_to_email($id)
    {
        ...
        try {
            ...
            $success = $this->creditos_model->send_invoice_to_client(
                $id,
                '',
        ...
        redirect(admin_url('creditos/list_creditos/' . $id));
    }
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        redirect(admin_url('creditos/list_creditos/' . $invoiceid));
    }
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('creditos/list_creditos'));
        }
        $success = $this->creditos_model->delete($id);
        ...
        if (strpos($_SERVER['HTTP_REFERER'], 'list_creditos') !== false) {
            redirect(admin_url('creditos/list_creditos'));
        } else {
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->creditos_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->creditos_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function mark_as_sent($id)
    {
        ...
        if (!user_can_view_credito($id)) {
            access_denied('Invoice Mark As Sent');
        }

        $success = $this->creditos_model->set_invoice_sent($id, true);
