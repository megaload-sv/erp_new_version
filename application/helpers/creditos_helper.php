 ...
 * @return string Url
 */
function get_credito_shortlink($invoice)
{
    ...
    $short_link = app_generate_short_link([
        'long_url' => $long_url,
        'title'    => format_credito_number($invoice->id),
    ]);
 ...
 * @param  mixed $credito_total
 * @return mixed  total left
 */
function get_credito_total_left_to_pay($id, $credito_total = null)
{
    $CI = &get_instance();

    if ($credito_total === null) {
        $CI->db->select('total')
            ->where('id', $id);
        $credito_total = $CI->db->get(db_prefix() . 'invoices')->row()->total;
    }
    ...
    $payments = $CI->payments_model->get_credito_payments($id);
    $credits  = $CI->credit_notes_model->get_applied_credito_credits($id);
    ...
    if (function_exists('bcsub')) {
        return bcsub($credito_total, $totalPayments, get_decimal_places());
    }

    return number_format($credito_total - $totalPayments, get_decimal_places(), '.', '');
}
 ...
 * @return boolean
 */
function is_creditos_email_overdue_notice_enabled()
{
 ...
 * @return boolean
 */
function is_creditos_email_due_notice_enabled()
{
 ...
 * @return boolean
 */
function is_creditos_overdue_reminders_enabled()
{
    return is_creditos_email_overdue_notice_enabled() || is_sms_trigger_active(SMS_TRIGGER_INVOICE_OVERDUE);
}
 ...
 * @return boolean
 */
function is_creditos_due_reminders_enabled()
{
    return is_creditos_email_due_notice_enabled() || is_sms_trigger_active(SMS_TRIGGER_INVOICE_DUE);
}
 ...
 */
function check_credito_restrictions($id, $hash)
{
    $CI = &get_instance();
    $CI->load->model('creditos_model');
    if (!$hash || !$id) {
        show_404();
    }
    ...
    $invoice = $CI->creditos_model->get($id);
 ...
 * @return mixed
 */
function format_credito_status($status, $classes = '', $label = true)
{
    if (!class_exists('Invoices_model', false)) {
        get_instance()->load->model('creditos_model');
    }

    $id          = $status;
    $label_class = get_credito_status_label($status);
 ...
 * @return string
 */
function get_credito_status_label($status)
{
    if (!class_exists('Invoices_model', false)) {
        get_instance()->load->model('creditos_model');
    }
 ...
 * @return boolean
 */
function is_credito_overdue($invoice)
{
    if (!class_exists('Invoices_model', false)) {
        get_instance()->load->model('creditos_model');
    }
 ...
 * @return string
 */
function credito_status_color_pdf($status_id)
{
    $statusColor = '';
 ...
 */
function update_credito_status($id, $force_update = false, $prevent_logging = false)
{
    $CI = &get_instance();
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        $log = 'Invoice Status Updated [Invoice Number: ' . format_credito_number($invoice->id) . ', From: ' . format_credito_status($original_status, '', false) . ' To: ' . format_credito_status($status, '', false) . ']';
        ...
        $CI->creditos_model->log_invoice_activity($invoice->id, 'invoice_activity_status_updated', false, $additional_activity);
 ...
 * @return string
 */
function format_credito_number($id)
{
    $CI = &get_instance();
 ...
 * @return array
 */
function get_credito_item_taxes($itemid)
{
    $CI = &get_instance();
 ...
 * @return boolean
 */
function is_payment_mode_allowed_for_credito($id, $invoiceid)
{
    $CI = &get_instance();
 ...
 * @return boolean
 */
function found_credito_mode($modes, $invoiceid, $offline = true, $show_on_pdf = false)
{
    $CI = &get_instance();
    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'invoices')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.currency', 'left')->where(db_prefix() . 'invoices.id', $invoiceid);
    $invoice = $CI->db->get()->row();
 ...
 * @param  mixed $total_creditos in case the total is calculated in other place
 * @return array
 */
function get_creditos_percent_by_status($status)
{
    $has_permission_view = has_permission('invoices', '', 'view');
    $total_invoices      = total_rows(db_prefix() . 'invoices', 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_creditos_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

    $data            = [];
    $total_by_status = 0;
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows(db_prefix() . 'invoices', 'sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND (' . get_creditos_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
        }
    } else {
        $total_by_status = total_rows(db_prefix() . 'invoices', 'status = ' . $status . ' AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_creditos_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
    }
 ...
 * @return boolean
 */
function staff_has_assigned_creditos($staff_id = '')
{
    $CI       = &get_instance();
 ...
 * @return string
 */
function load_creditos_total_template()
{
    $CI = &get_instance();
    $CI->load->model('creditos_model');
    $_data = $CI->input->post();
    ...
    $data['invoices_years'] = $CI->creditos_model->get_creditos_years();
    ...
    $data['total_result'] = $CI->creditos_model->get_creditos_total($_data);
    $data['_currency']    = $data['total_result']['currencyid'];

    $CI->load->view('admin/invoices/invoices_total_template', $data);
}

function get_creditos_where_sql_for_staff($staff_id)
{
    $CI                                 = &get_instance();
