...
class Creditos_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                ...
                $invoice->items       = get_items_by_type('credito', $id);
    ...
    {
        //Lo comento por que lo lleno desde antes de la llamda del metodo
        //$data['prefix'] = "CRE-";
        ...
        if ($insert_id) {
            ...
            if (!$this->is_draft($insert_id)) {
                ...
                //Aqui debo de hacer el incremento de los correlativos, dependiendo el tipo de factura
                $this->increment_next_number_values($data['prefix']);
            }

            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'credito')) {
            ...
            update_sales_total_tax_column($insert_id, 'credito', db_prefix() . 'invoices');
    ...
    public function update($data, $id)
    {
        ...
        foreach ($data['removed_items'] as $remove_item_id) {
            ...
            if (handle_removed_sales_item_post($remove_item_id, 'credito')) {
                $affectedRows++;
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                    if (delete_taxes_from_item($item['itemid'], 'credito')) {
                        $affectedRows++;
                    }
                } else {
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'credito')) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
                        $this->db->insert(db_prefix() . 'related_items', [
        ...
        if ($affectedRows > 0) {
            update_sales_total_tax_column($id, 'credito', db_prefix() . 'invoices');
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('relid IN (SELECT id from ' . db_prefix() . 'itemable WHERE rel_type="credito" AND rel_id="' . $this->db->escape_str($id) . '")');
            $this->db->where('fieldto', 'items');
            $this->db->delete(db_prefix() . 'customfieldsvalues');

            $items = get_items_by_type('credito', $id);
    ...
    public function change_invoice_number_when_status_draft($id)
    {
        ...
        $this->increment_next_number_values($invoice['prefix']);
    ...
    public function increment_next_number_values($tipoFactura){

        $valueFlag = "";
        if($tipoFactura =="FCF-"){
            $valueFlag = "next-number-fcf";
        }else if ($tipoFactura =="CRE-"){
            $valueFlag = "next-number-cre";
        }else if ($tipoFactura =="FEX-"){
            $valueFlag = "next-number-fex";
        }else if ($tipoFactura == "FSE-") {
            $valueFlag = "next-number-fse";
        }
        $this->db->where('name', $valueFlag);
        $this->db->set('value', 'value+1', false);
        $this->db->update(db_prefix() . 'options');

        $this->db->where('prefix', $tipoFactura);
        $this->db->set('next_number', 'next_number+1', false);
        $this->db->update(db_prefix() . 'invoice_custom_settings_values');
